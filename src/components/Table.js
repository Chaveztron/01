              ...
              {stableSort(rows, getComparator(order, orderBy))
                ...
                .map((row, index) => {
                  ...
                  return (
                    ...
                    {
                      headCells.map((propiedad, index) => {
                        let propi = propiedad.id
                        //console.log(row.propi)
                          if (index === 1) {
                            return   <TableCell key={`id_${row[propi]}+${index}`} component="th" id={labelId} scope="row" padding="none">{row[propi]}</TableCell>
                          }else{
                          return  <TableCell key={`id_${row[propi]}+${index}`} align="right">{row[propi]}</TableCell>;
                        }
                      })
                    }
